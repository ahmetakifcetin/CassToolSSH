...
## How to Create Screenshots for casstool
...
# It will automatically display casstool greeting
# Take screenshot
...
### Style Screenshots
For each style (casstool, minimal, blocks, simple):
...
STYLES="casstool minimal blocks simple"
...
```markdown
![casstool Demo](screenshots/main-demo.png)
...
sudo sed -i 's/$(hostname -f)/demo.casstool.local/' /opt/casstool/motd.sh

# Override IP display  
sudo sed -i 's/$(get_ip)/192.168.1.100/' /opt/casstool/motd.sh
sudo sed -i 's/$(get_public_ip)/203.0.113.42/' /opt/casstool/motd.sh
```
...
# Record
asciinema rec casstool-demo.cast

# Convert to GIF
# Use: https://github.com/asciinema/agg
agg casstool-demo.cast casstool-demo.gif
```
...
# Record
terminalizer record casstool-demo

# Render
terminalizer render casstool-demo
```
